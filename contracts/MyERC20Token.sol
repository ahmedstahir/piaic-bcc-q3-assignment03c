...
contract MyERC20Token is IERC20
{
    ...
    mapping (address => bool) _approvedForPriceChange;
    ...
    function returnToken(uint amount) external nonZeroAddress(msg.sender)
    {
        uint etherAmountToPay = amount * _perTokenWeiValue;
        require(address(this).balance >= etherAmountToPay, "Not enough balance in contract to pay");
        
        transferAmount(msg.sender, _ownerAddress, amount * 10 ** _decimal);
        
        payable(msg.sender).transfer(etherAmountToPay);
    }
    
    function setPerTokenWeiValue(uint tokenValueInWei) external
    {
        require(msg.sender == _ownerAddress || _approvedForPriceChange[msg.sender], "Only owner or an approved address can change price");
    ...
    function transferOwnership(address newOwner) external ownerOnly nonZeroAddress(newOwner)
    {
        _ownerAddress = newOwner;
    }
    
    function approveForPriceChange(address approver) external ownerOnly nonZeroAddress(approver)
    {
        _approvedForPriceChange[approver] = true;
    }
